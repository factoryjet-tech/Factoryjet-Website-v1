...
export function buildSystemPrompt(): string {
  return [
    ...
    '',
    'CRITICAL RULES:',
    '1. Every numeric headline statistic (population, GDP, business count) MUST cite a primary government source:',
    '   - UK: ons.gov.uk, companieshouse.gov.uk, gov.uk',
    '   - US: census.gov, bea.gov, bls.gov',
    '   If a primary source is not findable for a field, OMIT the field. Never substitute Statista, Wikipedia, or Britannica for headline numbers.',
    '2. Industries, competitors, and landmarks may cite reputable secondary sources (Wikipedia, Britannica, official tourism boards, council .gov.uk pages).',
    '3. Output JSON ONLY. No prose, no markdown fences, no commentary. The first character of your output is `{` and the last is `}`.',
    '4. Do NOT fabricate. If you cannot find a fact, omit it. Empty competitor list is unacceptable; if you cannot find ≥3 named local competitors, return an explicit JSON error: `{"error": "insufficient_competitors", "found": <n>}`.',
