#!/usr/bin/env python3
"""PHASE A — what do Indians ACTUALLY type/ask when shopping for our services?

Pulls real long-tail from DataForSEO Labs (keyword_suggestions + related_keywords),
India location, then classifies:
...
candidate list and report which ones actually carry Indian search volume.
...
SEEDS = [
    "seo agency", "seo company", "seo services", "ai seo",
    "web design company", "web development company", "website design agency",
    "ecommerce development company", "ecommerce agency", "shopify development",
    "ai agency", "ai agent development", "ai automation agency",
    "digital marketing agency", "generative engine optimization", "answer engine optimization",
]
...
if __name__ == "__main__":
    ...
    print(f"pulled {len(rows)} unique India keywords  cost=${cost:.3f}  errors={len(errs)}")
    ...
    print("\n=== SUPERLATIVES ACTUALLY USED (India, by total monthly volume) ===")
    ...
    out = os.path.join(HERE, "data", "india_demand.csv")
    ...
    json.dump({"superlative_tally": {k: v for k, v in tally.items()},
              ...
              open(os.path.join(HERE, "data", "india_demand_summary.json"), "w"), indent=1)
