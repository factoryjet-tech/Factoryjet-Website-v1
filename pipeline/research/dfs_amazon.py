#!/usr/bin/env python3
"""
Focused DataForSEO Labs pull for the Amazon Agency landing page (US).
Seeds cover the commercial agency head terms + the sell-on-amazon blog cluster.
Reads DATAFORSEO_LOGIN / DATAFORSEO_PASSWORD from pipeline/research/.env.
Writes data/amazon_keywords.csv and prints a compact, decision-ready summary.
"""
...
SEEDS = [
    ("sug", "amazon agency"),
    ("sug", "amazon advertising agency"),
    ("sug", "amazon marketing agency"),
    ("sug", "amazon ppc agency"),
    ("sug", "amazon seo agency"),
    ("sug", "amazon ppc management"),
    ("sug", "amazon account management"),
    ("sug", "amazon consultant"),
    ("sug", "amazon fba agency"),
    ("sug", "amazon listing optimization"),
    ("idea", "amazon marketing agency"),
    ("idea", "sell on amazon"),
    ("sug", "how to sell on amazon"),
]
...
def main():
    ...
    for ep, seed in SEEDS:
        resp = call(ep, seed)
        with open(os.path.join(RAW, f"amz__{ep}__{slug(seed)}.json"), "w") as f:
            json.dump(resp, f)
        if resp.get("_error"):
    ...
    out = os.path.join(HERE, "data", "amazon_keywords.csv")
    with open(out, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["keyword", "seed", "search_volume", "cpc", "competition", "kd", "intent"])
        w.writeheader(); w.writerows(recs)

    print(f"\n=== AMAZON RUN COMPLETE  cost=${cost:.3f}  unique={len(recs)}  csv={out}")
    ...
    show("COMMERCIAL / AGENCY TERMS (agency|advertising|ppc|marketing|management|consultant|seo|fba), any KD",
         lambda r: any(t in r["keyword"] for t in ["agency", "advertis", "ppc", "marketing", "management", "consultant", "seo", "fba", "account manag"]))
    ...
    show("BLOG / INFORMATIONAL CLUSTER (how|what|without inventory|start|cost|fees)",
         lambda r: any(t in r["keyword"] for t in ["how ", "what ", "without inventory", "start sell", "cost to sell", "fba fees", "seller", "dropship"]))
