...
description: Strip invisible Unicode watermarks (zero-width chars, exotic spaces, bidi controls, tag chars) and AI-tell phrasing from prose we own before it ships. Use whenever writing, editing, or auditing user-facing copy on this site - blog posts, page copy, metadata, FAQ answers, JSON-LD text - and always as the last pass before commit or publish. Do not use for code-only tasks; leave code, commands, identifiers, paths, APIs, URLs, formulas, citations, and verbatim quotations unchanged.
---
